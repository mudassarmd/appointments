...
{ 
    ...
    function __construct(){ 
        $this->EA =& get_instance(); 
        $this->EA->load->helper('url'); 
        $this->EA->load->helper('form'); 
        $this->EA->load->config('paypal'); 
         
        $sanbox = $this->EA->config->item('sandbox'); 
        $this->paypal_url = ($sanbox == TRUE)?'https://www.sandbox.paypal.com/cgi-bin/webscr':'https://www.paypal.com/cgi-bin/webscr'; 
                 
        $this->last_error = ''; 
        $this->ipn_response = ''; 
 
        // $this->ipn_log_file = $this->EA->config->item('paypal_lib_ipn_log_file'); 
        // $this->ipn_log = $this->EA->config->item('paypal_lib_ipn_log');  
         
        $this->button_path = $this->EA->config->item('paypal_lib_button_path'); 
         
        // populate $fields array with a few default values. 
        // values can be overwritten by the calling script. 
        $businessEmail = $this->EA->config->item('business'); 
        $this->add_field('business',$businessEmail); 
        $this->add_field('rm','2');    // Return method = POST 
        $this->add_field('cmd','_xclick'); 
 
        $this->add_field('currency_code', $this->EA->config->item('paypal_lib_currency_code')); 
    ...
    function paypal_auto_form(){ 
        ...
        $html .='<body style="text-align:center;" onLoad="document.forms[\'paypal_auto_form\'].submit();">' . "\n"; 
        $html .='<p style="text-align:center;">Please wait, your order is being processed and you will be redirected to the PayPal website.</p>' . "\n"; 
        $html .=$this->paypal_form('paypal_auto_form'); 
        $html .='</body></html>'; 
    ...
    function paypal_form($form_name='paypal_form'){ 
        ...
        $str .= '<p><input type="submit" class="btn btn-success btn-lg btn-block" value="Pay Now" /></p>'; 
        $str .= form_close() . "\n"; 
